...
(async () => {
  const __mfHostInit = await __mfImport("./hostInit-BgitNuSp.js");
  await __mfHostInit.__tla;
  const { initHost } = __mfHostInit;
  
  const runtime = await initHost();
  const __mfPreloadRemote = (runtimeRemote, remote) => {
    const remoteCacheKey = "virtual:mf:__mfe_internal__host__mf_owner__1__mf_v__runtimeInit__mf_v__.js::" + remote;
  ...
  const __mfRemotePreloads = [__mfPreloadRemote("__mfe_internal__host__mf_owner__1__cart", "cart"),__mfPreloadRemote("__mfe_internal__host__mf_owner__1__products", "products")];
...
})().then(() => __mfImport("./index-B5TJUbN9.js"));
